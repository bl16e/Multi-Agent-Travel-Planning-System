...
Default mode is offline-safe. If `QWEN_API_KEY` is unset, structured agent calls return deterministic fallback data and bureau outputs are marked as estimates that did not use real-time data.
...
Generated Markdown and iCalendar files are written under `OUTPUT_DIR` or the configured artifact directory. New planning requests require request IDs to match `[A-Za-z0-9_.-]{1,100}`. Internal file access still sanitizes IDs before building paths.
